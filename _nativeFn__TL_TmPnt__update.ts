...
let mg_nativeFn__clgVarRt__TL_TmPnt__update:NativeFunction<void,[ThreadId,MG_TmPntVal]>  |null;  
...
function call_nativeFn__TL_TmPnt__update(curThreadId:ThreadId,tmPntVal:MG_TmPntVal){
  ...
  if(mg_nativeFn__clgVarRt__TL_TmPnt__update){
    // const ret= //无返回值
    mg_nativeFn__clgVarRt__TL_TmPnt__update(curThreadId,tmPntVal);
  }
...
function get_nativeFn__clgVarRt__TL_TmPnt__update():void{
  ...
  mg_nativeFn__clgVarRt__TL_TmPnt__update=  new NativeFunction(fnAdr__clgVarRt__TL_TmPnt__update, 'void',['int','int']);
}
